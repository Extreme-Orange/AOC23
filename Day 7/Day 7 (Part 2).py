...
for card in cards_bids:
    ...
    for item in bag:
        if item == 'J':
            if bag[item] < card_type[0]:
                card_type[0] += bag[item]
                card_type.remove(bag[item])
            elif bag[item] == card_type[0]:
                if card_type[0] == 5:
                    continue
                else:
                    card_type[1] += bag[item]
                    card_type.remove(bag[item])
            elif bag[item] == card_type[0] and bag[item] == card_type[1]:
                card_type[0] += bag[item]
                card_type.remove(bag[item])
        else:
            continue
...
def sort_cards(x):
    cheat_sheet = {'A':13, 'K':12, 'Q':11, 'T':10, '9':9, '8':8, '7':7, '6':6, '5':5, '4':4, '3':3, '2':2, 'J':1}
    points = 0
    pos = 10000000000
    for letter in x:
